...
const AdminPage = () => {
  ...
  const handleDeleteEntity = async () => {
    ...
    if (!window.confirm(`Delete "${currentEntity.name}"? This cannot be undone.`)) return;

    const updatedData = { ...worldData };
    updatedData.entries = updatedData.entries.filter(e => e.id !== currentEntityId);

    // Also remove from any subfolder entries arrays
    if (updatedData.subfolders) {
      for (const catKey of Object.keys(updatedData.subfolders)) {
        updatedData.subfolders[catKey] = updatedData.subfolders[catKey].map(folder => ({
          ...folder,
          entries: (folder.entries || []).filter(eid => eid !== currentEntityId)
        }));
      }
    }
              ...
              <>
                {editMode ? (
                  ...
                  <>
                    ...
                    <button className="tb-btn" style={{color: 'var(--red)'}} onClick={handleDeleteEntity}>
